...
public abstract class CallBackUtil<T> {
    ...
    public static abstract class CallBackFile extends CallBackUtil<File> {
         ...
         * @param fileName：文件名
         */
        public CallBackFile(String fileDir, String fileName) {
            mDestFileDir = fileDir;
        ...
        public File onParseResponse(Call call, Response response) {
            ...
            try {
                ...
                while ((len = is.read(buf)) != -1) {
                    ...
                    // 优化进度显示
                    final int tempProgress = (int) (finalSum * 100.0f / total);
                    if (tempProgress >= 0 && tempProgress > progress) {
                        progress = tempProgress;
                        mMainHandler.post(new Runnable() {
                            @Override
                            public void run() {
                                onProgress(tempProgress, total);
                            }
                        });
                    }
                }
